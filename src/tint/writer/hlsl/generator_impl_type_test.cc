...
TEST_F(HlslGeneratorImplTest_Type, EmitType_StructDecl_OmittedIfStorageBuffer) {
    ...
    GlobalVar("g", ty.Of(s), type::AddressSpace::kStorage, ast::Access::kReadWrite, Binding(0_a),
              Group(0_a));
...
TEST_F(HlslGeneratorImplTest_Type, EmitSampler) {
    ...
    std::stringstream out;
    ASSERT_TRUE(gen.EmitType(out, sampler, type::AddressSpace::kNone, ast::Access::kReadWrite, ""))
        << gen.error();
