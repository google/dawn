...
[numthreads(1, 1, 1)]
void f() {
  uint v_17 = (32u * min(uint(i()), 3u));
  uint v_18 = (8u * min(uint(i()), 3u));
  matrix<float16_t, 4, 3> l_a[4] = v_13(0u);
  matrix<float16_t, 4, 3> l_a_i = v_5(v_17);
  uint4 v_19 = a[((v_17 + v_18) / 16u)];
  vector<float16_t, 3> l_a_i_i = tint_bitcast_to_f16(((((((v_17 + v_18) % 16u) / 4u) == 2u)) ? (v_19.zw) : (v_19.xy))).xyz;
  uint v_20 = a[((v_17 + v_18) / 16u)][(((v_17 + v_18) % 16u) / 4u)];
