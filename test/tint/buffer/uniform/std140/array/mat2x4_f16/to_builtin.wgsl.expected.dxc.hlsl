...
[numthreads(1, 1, 1)]
void f() {
  matrix<float16_t, 4, 2> t = transpose(v_2(32u));
  float16_t l = length(tint_bitcast_to_f16_1(u[0u].zw).ywxz);
  float16_t a = abs(tint_bitcast_to_f16(u[0u].z).y);
  float16_t v_7 = (t[0u].x + float16_t(l));
  s.Store<float16_t>(0u, (v_7 + float16_t(a)));
}
