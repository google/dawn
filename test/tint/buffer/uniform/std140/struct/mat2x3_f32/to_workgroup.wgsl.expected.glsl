...
S v_2(uint start_byte_offset) {
  ...
  mat2x3 v_5 = v_1((16u + start_byte_offset));
  uvec4 v_6 = v.inner[((64u + start_byte_offset) / 16u)];
  return S(v_4, v_5, int(v_6[(((64u + start_byte_offset) & 15u) >> 2u)]));
}
S[4] v_7(uint start_byte_offset) {
  S a[4] = S[4](S(0, mat2x3(vec3(0.0f), vec3(0.0f)), 0), S(0, mat2x3(vec3(0.0f), vec3(0.0f)), 0), S(0, mat2x3(vec3(0.0f), vec3(0.0f)), 0), S(0, mat2x3(vec3(0.0f), vec3(0.0f)), 0));
  {
    uint v_8 = 0u;
    v_8 = 0u;
    while(true) {
...
void f_inner(uint tint_local_index) {
  {
    ...
    while(true) {
      ...
      w[v_11] = S(0, mat2x3(vec3(0.0f), vec3(0.0f)), 0);
      {
        v_10 = (v_11 + 1u);
      }
      continue;
  ...
  w[1u] = v_2(256u);
  w[3u].m = v_1(272u);
  w[1u].m[0u] = uintBitsToFloat(v.inner[2u].xyz).zxy;
}
