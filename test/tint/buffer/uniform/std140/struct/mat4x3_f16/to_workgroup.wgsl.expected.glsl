...
f16mat4x3 v_1(uint start_byte_offset) {
  ...
  f16vec3 v_7 = tint_bitcast_to_f16(mix(v_6.xy, v_6.zw, bvec2(((((16u + start_byte_offset) % 16u) / 4u) == 2u)))).xyz;
  uvec4 v_8 = v.inner[((24u + start_byte_offset) / 16u)];
  return f16mat4x3(v_3, v_5, v_7, tint_bitcast_to_f16(mix(v_8.xy, v_8.zw, bvec2(((((24u + start_byte_offset) % 16u) / 4u) == 2u)))).xyz);
}
S v_9(uint start_byte_offset) {
  uvec4 v_10 = v.inner[(start_byte_offset / 16u)];
  int v_11 = int(v_10[((start_byte_offset % 16u) / 4u)]);
  f16mat4x3 v_12 = v_1((8u + start_byte_offset));
  uvec4 v_13 = v.inner[((64u + start_byte_offset) / 16u)];
  return S(v_11, v_12, int(v_13[(((64u + start_byte_offset) % 16u) / 4u)]));
}
...
void f_inner(uint tint_local_index) {
  {
    ...
    while(true) {
      ...
      w[v_18] = S(0, f16mat4x3(f16vec3(0.0hf), f16vec3(0.0hf), f16vec3(0.0hf), f16vec3(0.0hf)), 0);
      {
        v_17 = (v_18 + 1u);
      }
      continue;
  ...
  w[1u] = v_9(256u);
  w[3u].m = v_1(264u);
  w[1u].m[0u] = tint_bitcast_to_f16(v.inner[1u].xy).xyz.zxy;
}
