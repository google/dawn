...
matrix<float16_t, 3, 4> v_2(uint start_byte_offset) {
  ...
  uint4 v_9 = u[(v_8 / 16u)];
  return matrix<float16_t, 3, 4>(v_4, v_7, tint_bitcast_to_f16_1(select((((v_8 & 15u) >> 2u) == 2u), v_9.zw, v_9.xy)));
}

S v_10(uint start_byte_offset) {
  int v_11 = asint(u[(start_byte_offset / 16u)][((start_byte_offset & 15u) >> 2u)]);
  matrix<float16_t, 3, 4> v_12 = v_2((8u + start_byte_offset));
...
void f() {
  ...
  b(v_21);
  c(v_2(264u));
  d(tint_bitcast_to_f16_1(u[1u].xy).ywxz);
  e(tint_bitcast_to_f16(u[1u].x).y);
}
