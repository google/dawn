...
f16mat3x4 v_1(uint start_byte_offset) {
  ...
  return f16mat3x4(v_3, v_6, tint_bitcast_to_16bit_1(mix(v_8.xy, v_8.zw, bvec2((((v_7 & 15u) >> 2u) == 2u)))));
}
layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;
void main() {
  f16mat4x3 t = transpose(v_1(0u));
  float16_t l = length(tint_bitcast_to_16bit_1(v.inner[0u].zw));
  uvec4 v_9 = v.inner[0u];
  float16_t a = abs(tint_bitcast_to_16bit(v_9.x).y);
}
