...
matrix<float16_t, 4, 2> v_1(uint start_byte_offset) {
  ...
  vector<float16_t, 2> v_6 = tint_bitcast_to_f16(m[(v_5 / 16u)][((v_5 & 15u) >> 2u)]);
  uint v_7 = (12u + start_byte_offset);
  return matrix<float16_t, 4, 2>(v_2, v_4, v_6, tint_bitcast_to_f16(m[(v_7 / 16u)][((v_7 & 15u) >> 2u)]));
}

[numthreads(1, 1, 1)]
void f() {
  uint v_8 = (min(uint(i()), 3u) * 4u);
  matrix<float16_t, 4, 2> l_m = v_1(0u);
  vector<float16_t, 2> l_m_i = tint_bitcast_to_f16(m[(v_8 / 16u)][((v_8 & 15u) >> 2u)]);
}
