...
matrix<float16_t, 2, 3> v_1(uint start_byte_offset) {
  ...
  uint4 v_5 = u[(v_4 / 16u)];
  return matrix<float16_t, 2, 3>(v_3, tint_bitcast_to_f16_1(select((((v_4 & 15u) >> 2u) == 2u), v_5.zw, v_5.xy)).xyz);
}

[numthreads(1, 1, 1)]
void f() {
  a(v_1(0u));
  b(tint_bitcast_to_f16_1(u[0u].zw).xyz);
  b(tint_bitcast_to_f16_1(u[0u].zw).xyz.zxy);
  c(tint_bitcast_to_f16(u[0u].z).x);
  c(tint_bitcast_to_f16(u[0u].w).x);
}
