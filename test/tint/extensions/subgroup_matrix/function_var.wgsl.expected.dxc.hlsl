...
void main() {
  ...
  m.Store(buffer, (0u + (select(v_7, v_5, 0u) * 4u)), (select(v_7, v_6, 8u) * 4u), MatrixLayout::RowMajor);
  uint v_8 = 0u;
  buffer.GetDimensions(v_8);
  uint v_9 = asuint(int(0));
  uint v_10 = asuint(int(64));
  bool v_11 = (((v_9 + (v_10 * 7u)) + 8u) <= (v_8 / 4u));
  m_array[0u].Store(buffer, (0u + (select(v_11, v_9, 0u) * 4u)), (select(v_11, v_10, 8u) * 4u), MatrixLayout::RowMajor);
  uint v_12 = 0u;
  buffer.GetDimensions(v_12);
  uint v_13 = asuint(int(0));
  uint v_14 = asuint(int(64));
  bool v_15 = (((v_13 + (v_14 * 7u)) + 8u) <= (v_12 / 4u));
  m_nested_array[1u][2u].Store(buffer, (0u + (select(v_15, v_13, 0u) * 4u)), (select(v_15, v_14, 8u) * 4u), MatrixLayout::RowMajor);
  uint v_16 = 0u;
  buffer.GetDimensions(v_16);
  uint v_17 = asuint(int(0));
  uint v_18 = asuint(int(64));
  bool v_19 = (((v_17 + (v_18 * 7u)) + 8u) <= (v_16 / 4u));
  m_struct.l.Store(buffer, (0u + (select(v_19, v_17, 0u) * 4u)), (select(v_19, v_18, 8u) * 4u), MatrixLayout::RowMajor);
  uint v_20 = 0u;
  buffer.GetDimensions(v_20);
  uint v_21 = asuint(int(0));
  uint v_22 = asuint(int(64));
  bool v_23 = (((v_21 + (v_22 * 7u)) + 8u) <= (v_20 / 4u));
  m_nested_struct.s.r.Store(buffer, (0u + (select(v_23, v_21, 0u) * 4u)), (select(v_23, v_22, 8u) * 4u), MatrixLayout::RowMajor);
}
