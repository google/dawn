...
void main() {
  ...
  m_array_let[int(0)].Store(buffer, (0u + (select(v_7, 0u, 0u) * 4u)), (select(v_7, 64u, 8u) * 4u), MatrixLayout::RowMajor);
  uint v_8 = 0u;
  buffer.GetDimensions(v_8);
  bool v_9 = (456u <= (v_8 / 4u));
  m_nested_array_let[int(1)][int(2)].Store(buffer, (0u + (select(v_9, 0u, 0u) * 4u)), (select(v_9, 64u, 8u) * 4u), MatrixLayout::RowMajor);
  uint v_10 = 0u;
