struct tint_TransferFunctionParams {
  uint mode;
  float A;
  float B;
  float C;
  float D;
  float E;
  float F;
  float G;
};

struct tint_ExternalTextureParams {
  uint numPlanes;
  uint doYuvToRgbConversionOnly;
  float3x4 yuvToRgbConversionMatrix;
  tint_TransferFunctionParams srcTransferFunction;
  tint_TransferFunctionParams dstTransferFunction;
  float3x3 gamutConversionMatrix;
  float3x2 sampleTransform;
  float3x2 loadTransform;
  float2 samplePlane0RectMin;
  float2 samplePlane0RectMax;
  float2 samplePlane1RectMin;
  float2 samplePlane1RectMax;
  uint2 apparentSize;
  float2 plane1CoordFactor;
  float4 ootfParam;
};


cbuffer cbuffer_t_f_params : register(b2) {
  uint4 t_f_params[18];
};
Texture2D<float4> t_f_plane0 : register(t0);
Texture2D<float4> t_f_plane1 : register(t1);
RWByteAddressBuffer v_1 : register(u0, space1);
uint2 tint_v2f32_to_v2u32(float2 value) {
  return uint2(clamp(value, (0.0f).xx, (4294967040.0f).xx));
}

float3 tint_ApplyGammaTransferFunction(float3 v, tint_TransferFunctionParams params) {
  float3 v_2 = float3((params.G).xxx);
  float3 v_3 = float3((params.D).xxx);
  float3 v_4 = abs(v);
  float3 v_5 = float3(sign(v));
  return select((v_4 < v_3), (v_5 * ((params.C * v_4) + params.F)), (v_5 * (pow(((params.A * v_4) + params.B), v_2) + params.E)));
}

float tint_ApplyHLGSingleChannel(float v, tint_TransferFunctionParams params) {
  if ((v <= params.D)) {
    return ((v * v) / params.E);
  } else {
    return ((params.B + exp(((v - params.C) / params.A))) / params.F);
  }
  /* unreachable */
  return 0.0f;
}

float3 tint_ApplyHLGTransferFunction(float3 v, tint_TransferFunctionParams params) {
  float v_6 = tint_ApplyHLGSingleChannel(v.x, params);
  float v_7 = tint_ApplyHLGSingleChannel(v.y, params);
  return float3(v_6, v_7, tint_ApplyHLGSingleChannel(v.z, params));
}

float3 tint_ApplyPQTransferFunction(float3 v, tint_TransferFunctionParams params) {
  float3 v_8 = float3((params.C).xxx);
  float3 v_9 = float3((params.D).xxx);
  float3 v_10 = float3((params.E).xxx);
  float3 v_11 = float3((params.A).xxx);
  float3 v_12 = pow(clamp(v, (0.0f).xxx, (1.0f).xxx), ((1.0f).xxx / float3((params.B).xxx)));
  return pow((max((v_12 - v_8), (0.0f).xxx) / (v_9 - (v_10 * v_12))), ((1.0f).xxx / v_11));
}

float3 tint_ApplySrcTransferFunction(float3 v, tint_TransferFunctionParams params) {
  if ((params.mode == 0u)) {
    return tint_ApplyGammaTransferFunction(v, params);
  } else {
    if ((params.mode == 1u)) {
      return tint_ApplyHLGTransferFunction(v, params);
    } else {
      return tint_ApplyPQTransferFunction(v, params);
    }
    /* unreachable */
    return (0.0f).xxx;
  }
  /* unreachable */
  return (0.0f).xxx;
}

float4 tint_TextureLoadMultiplanarExternal(Texture2D<float4> plane_0, Texture2D<float4> plane_1, tint_ExternalTextureParams params, uint2 coords) {
  float2 v_13 = round(mul(float3(float2(min(coords, params.apparentSize)), 1.0f), params.loadTransform));
  uint2 v_14 = tint_v2f32_to_v2u32(v_13);
  float3 v_15 = (0.0f).xxx;
  float v_16 = 0.0f;
  if ((params.numPlanes == 1u)) {
    float4 v_17 = plane_0.Load(int3(int2(v_14), int(0)));
    v_15 = v_17.xyz;
    v_16 = v_17.w;
  } else {
    float v_18 = plane_0.Load(int3(int2(v_14), int(0))).x;
    v_15 = mul(params.yuvToRgbConversionMatrix, float4(v_18, plane_1.Load(int3(int2(tint_v2f32_to_v2u32((v_13 * params.plane1CoordFactor))), int(0))).xy, 1.0f));
    v_16 = 1.0f;
  }
  float3 v_19 = v_15;
  float3 v_20 = (0.0f).xxx;
  if ((params.doYuvToRgbConversionOnly == 0u)) {
    tint_TransferFunctionParams v_21 = params.srcTransferFunction;
    float3 v_22 = tint_ApplySrcTransferFunction(v_19, v_21);
    float3 v_23 = (0.0f).xxx;
    if ((params.ootfParam.w != 0.0f)) {
      float v_24 = dot(params.ootfParam.xyz, v_22);
      v_23 = (v_22 * (float(sign(v_24)) * pow(abs(v_24), params.ootfParam.w)));
    } else {
      v_23 = v_22;
    }
    tint_TransferFunctionParams v_25 = params.dstTransferFunction;
    v_20 = tint_ApplyGammaTransferFunction(mul(v_23, params.gamutConversionMatrix), v_25);
  } else {
    v_20 = v_19;
  }
  return float4(v_20, v_16);
}

float3x2 v_26(uint start_byte_offset) {
  uint4 v_27 = t_f_params[(start_byte_offset / 16u)];
  uint v_28 = (8u + start_byte_offset);
  uint4 v_29 = t_f_params[(v_28 / 16u)];
  uint v_30 = (16u + start_byte_offset);
  uint4 v_31 = t_f_params[(v_30 / 16u)];
  return float3x2(asfloat(select((((start_byte_offset & 15u) >> 2u) == 2u), v_27.zw, v_27.xy)), asfloat(select((((v_28 & 15u) >> 2u) == 2u), v_29.zw, v_29.xy)), asfloat(select((((v_30 & 15u) >> 2u) == 2u), v_31.zw, v_31.xy)));
}

float3x3 v_32(uint start_byte_offset) {
  return float3x3(asfloat(t_f_params[(start_byte_offset / 16u)].xyz), asfloat(t_f_params[((16u + start_byte_offset) / 16u)].xyz), asfloat(t_f_params[((32u + start_byte_offset) / 16u)].xyz));
}

tint_TransferFunctionParams v_33(uint start_byte_offset) {
  uint v_34 = (4u + start_byte_offset);
  uint v_35 = (8u + start_byte_offset);
  uint v_36 = (12u + start_byte_offset);
  uint v_37 = (16u + start_byte_offset);
  uint v_38 = (20u + start_byte_offset);
  uint v_39 = (24u + start_byte_offset);
  uint v_40 = (28u + start_byte_offset);
  tint_TransferFunctionParams v_41 = {t_f_params[(start_byte_offset / 16u)][((start_byte_offset & 15u) >> 2u)], asfloat(t_f_params[(v_34 / 16u)][((v_34 & 15u) >> 2u)]), asfloat(t_f_params[(v_35 / 16u)][((v_35 & 15u) >> 2u)]), asfloat(t_f_params[(v_36 / 16u)][((v_36 & 15u) >> 2u)]), asfloat(t_f_params[(v_37 / 16u)][((v_37 & 15u) >> 2u)]), asfloat(t_f_params[(v_38 / 16u)][((v_38 & 15u) >> 2u)]), asfloat(t_f_params[(v_39 / 16u)][((v_39 & 15u) >> 2u)]), asfloat(t_f_params[(v_40 / 16u)][((v_40 & 15u) >> 2u)])};
  return v_41;
}

float3x4 v_42(uint start_byte_offset) {
  return float3x4(asfloat(t_f_params[(start_byte_offset / 16u)]), asfloat(t_f_params[((16u + start_byte_offset) / 16u)]), asfloat(t_f_params[((32u + start_byte_offset) / 16u)]));
}

tint_ExternalTextureParams v_43(uint start_byte_offset) {
  uint v_44 = t_f_params[(start_byte_offset / 16u)][((start_byte_offset & 15u) >> 2u)];
  uint v_45 = (4u + start_byte_offset);
  uint v_46 = t_f_params[(v_45 / 16u)][((v_45 & 15u) >> 2u)];
  float3x4 v_47 = v_42((16u + start_byte_offset));
  tint_TransferFunctionParams v_48 = v_33((64u + start_byte_offset));
  tint_TransferFunctionParams v_49 = v_33((96u + start_byte_offset));
  float3x3 v_50 = v_32((128u + start_byte_offset));
  float3x2 v_51 = v_26((176u + start_byte_offset));
  float3x2 v_52 = v_26((200u + start_byte_offset));
  uint v_53 = (224u + start_byte_offset);
  uint4 v_54 = t_f_params[(v_53 / 16u)];
  uint v_55 = (232u + start_byte_offset);
  uint4 v_56 = t_f_params[(v_55 / 16u)];
  uint v_57 = (240u + start_byte_offset);
  uint4 v_58 = t_f_params[(v_57 / 16u)];
  uint v_59 = (248u + start_byte_offset);
  uint4 v_60 = t_f_params[(v_59 / 16u)];
  uint v_61 = (256u + start_byte_offset);
  uint4 v_62 = t_f_params[(v_61 / 16u)];
  uint v_63 = (264u + start_byte_offset);
  uint4 v_64 = t_f_params[(v_63 / 16u)];
  tint_ExternalTextureParams v_65 = {v_44, v_46, v_47, v_48, v_49, v_50, v_51, v_52, asfloat(select((((v_53 & 15u) >> 2u) == 2u), v_54.zw, v_54.xy)), asfloat(select((((v_55 & 15u) >> 2u) == 2u), v_56.zw, v_56.xy)), asfloat(select((((v_57 & 15u) >> 2u) == 2u), v_58.zw, v_58.xy)), asfloat(select((((v_59 & 15u) >> 2u) == 2u), v_60.zw, v_60.xy)), select((((v_61 & 15u) >> 2u) == 2u), v_62.zw, v_62.xy), asfloat(select((((v_63 & 15u) >> 2u) == 2u), v_64.zw, v_64.xy)), asfloat(t_f_params[((272u + start_byte_offset) / 16u)])};
  return v_65;
}

[numthreads(1, 1, 1)]
void main() {
  tint_ExternalTextureParams v_66 = v_43(0u);
  v_1.Store4(0u, asuint(tint_TextureLoadMultiplanarExternal(t_f_plane0, t_f_plane1, v_66, (0u).xx)));
}

