...
vec4 tint_TextureLoadMultiplanarExternal(tint_ExternalTextureParams params, uvec2 coords) {
  ...
  if ((params.numPlanes == 1u)) {
    vec4 v_16 = texelFetch(t_f_plane0, ivec2(v_13), 0);
    v_14 = v_16.xyz;
    v_15 = v_16.w;
  } else {
    float v_17 = texelFetch(t_f_plane0, ivec2(v_13), 0).x;
    v_14 = (vec4(v_17, texelFetch(t_f_plane1, ivec2(uvec2((v_12 * params.plane1CoordFactor))), 0).xy, 1.0f) * params.yuvToRgbConversionMatrix);
    v_15 = 1.0f;
  }
  vec3 v_18 = v_14;
  vec3 v_19 = vec3(0.0f);
...
void main() {
  ...
  vec4 vals = tint_TextureLoadMultiplanarExternal(v_77, min(uvec2(0u), ((v_77.apparentSize + uvec2(1u)) - uvec2(1u))));
}
