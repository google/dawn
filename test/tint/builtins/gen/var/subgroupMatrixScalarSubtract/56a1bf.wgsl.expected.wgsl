...
fn subgroupMatrixScalarSubtract_56a1bf() -> subgroup_matrix_left<f32, 8, 8> {
  ...
  var arg_1 = 8.0f;
  var res : subgroup_matrix_left<f32, 8, 8> = subgroupMatrixScalarSubtract(arg_0, arg_1);
  return res;
}

@compute @workgroup_size(1)
fn compute_main() {
  subgroupMatrixStore<row_major>(&(prevent_dce), 0, subgroupMatrixScalarSubtract_56a1bf(), 16);
}
