...
fn subgroupMatrixScalarMultiply_decf0b() -> subgroup_matrix_right<f32, 8, 8> {
  ...
  var arg_1 = 8.0f;
  var res : subgroup_matrix_right<f32, 8, 8> = subgroupMatrixScalarMultiply(arg_0, arg_1);
  return res;
}

@compute @workgroup_size(1)
fn compute_main() {
  subgroupMatrixStore<row_major>(&(prevent_dce), 0, subgroupMatrixScalarMultiply_decf0b(), 16);
}
