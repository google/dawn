...
layout(binding = 1, rgba16_snorm) uniform highp readonly image2D f_arg_0;
uvec2 textureDimensions_dd189c() {
  uvec2 res = uvec2(imageSize(f_arg_0));
  return res;
}
void main() {
  v.inner = textureDimensions_dd189c();
}
//
// compute_main
//
...
layout(binding = 1, rgba16_snorm) uniform highp readonly image2D arg_0;
uvec2 textureDimensions_dd189c() {
...
layout(binding = 0, rgba16_snorm) uniform highp readonly image2D v_arg_0;
...
VertexOutput vertex_main_inner() {
  ...
  v.pos = vec4(0.0f);
  v.prevent_dce = textureDimensions_dd189c();
  return v;
}
void main() {
