...
Failed to generate: :18:14 error: glsl.textureSize: no matching call to 'glsl.textureSize(texel_buffer<rgba32float, read>, i32)'
...
$B1: {  # root
  ...
  %arg_0:ptr<handle, texel_buffer<rgba32float, read>, read> = combined_texture_sampler undef @binding_point(0, 1)
}

%textureDimensions_ced70a = func():u32 {
  $B2: {
    %4:texel_buffer<rgba32float, read> = load %arg_0
    %5:i32 = glsl.textureSize %4, 0i
    %6:u32 = bitcast %5
...
%fragment_main = @fragment func():void {
  $B3: {
    %10:u32 = call %textureDimensions_ced70a
...
Failed to generate: :13:14 error: glsl.textureSize: no matching call to 'glsl.textureSize(texel_buffer<rgba32float, read>, i32)'
...
$B1: {  # root
  %arg_0:ptr<handle, texel_buffer<rgba32float, read>, read> = combined_texture_sampler undef @binding_point(0, 0)
}

%textureDimensions_ced70a = func():u32 {
  $B2: {
    %3:texel_buffer<rgba32float, read> = load %arg_0
    %4:i32 = glsl.textureSize %3, 0i
    %5:u32 = bitcast %4
...
%vertex_main = @vertex func():VertexOutput {
  $B3: {
    ...
    %12:u32 = call %textureDimensions_ced70a
    store %11, %12
