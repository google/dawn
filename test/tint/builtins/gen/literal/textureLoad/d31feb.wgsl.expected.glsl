...
layout(binding = 1, rgb10_a2ui) uniform highp readonly uimage3D f_arg_0;
uvec4 textureLoad_d31feb() {
  uvec4 res = imageLoad(f_arg_0, ivec3(min(uvec3(1u), (uvec3(imageSize(f_arg_0)) - uvec3(1u)))));
  return res;
}
void main() {
  v.inner = textureLoad_d31feb();
}
//
// compute_main
//
...
layout(binding = 1, rgb10_a2ui) uniform highp readonly uimage3D arg_0;
uvec4 textureLoad_d31feb() {
...
layout(binding = 0, rgb10_a2ui) uniform highp readonly uimage3D v_arg_0;
...
VertexOutput vertex_main_inner() {
  ...
  v.pos = vec4(0.0f);
  v.prevent_dce = textureLoad_d31feb();
  return v;
}
void main() {
