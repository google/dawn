...
layout(binding = 1, rg16ui) uniform highp readonly uimage3D f_arg_0;
uvec4 textureLoad_67b8cd() {
  uvec3 v_1 = (uvec3(imageSize(f_arg_0)) - uvec3(1u));
  uvec4 res = imageLoad(f_arg_0, ivec3(min(uvec3(ivec3(1)), v_1)));
  return res;
}
void main() {
  v.inner = textureLoad_67b8cd();
}
//
// compute_main
//
...
layout(binding = 1, rg16ui) uniform highp readonly uimage3D arg_0;
uvec4 textureLoad_67b8cd() {
...
layout(binding = 0, rg16ui) uniform highp readonly uimage3D v_arg_0;
...
VertexOutput vertex_main_inner() {
  ...
  v_1.pos = vec4(0.0f);
  v_1.prevent_dce = textureLoad_67b8cd();
  return v_1;
}
void main() {
