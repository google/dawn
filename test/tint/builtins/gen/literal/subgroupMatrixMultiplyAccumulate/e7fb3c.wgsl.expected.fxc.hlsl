SKIP: FAILED

$B1: {  # root
  %prevent_dce:ptr<storage, array<f32, 1024>, read_write> = var undef @binding_point(0, 0)
}

%subgroupMatrixMultiplyAccumulate_e7fb3c = func():subgroup_matrix_result<f32, 8, 8> {
  $B2: {
    %3:subgroup_matrix_left<f32, 8, 8> = construct
    %4:subgroup_matrix_right<f32, 8, 8> = construct
    %5:subgroup_matrix_result<f32, 8, 8> = construct
    %6:subgroup_matrix_result<f32, 8, 8> = subgroupMatrixMultiplyAccumulate %3, %4, %5
    %res:ptr<function, subgroup_matrix_result<f32, 8, 8>, read_write> = var %6
    %8:subgroup_matrix_result<f32, 8, 8> = load %res
    ret %8
  }
}
%compute_main = @compute @workgroup_size(1i, 1i, 1i) func():void {
  $B3: {
    %10:subgroup_matrix_result<f32, 8, 8> = call %subgroupMatrixMultiplyAccumulate_e7fb3c
    %11:void = subgroupMatrixStore %prevent_dce, 0i, %10, false, 8i
    ret
  }
}
Failed to generate: subgroup matrices support requires DXC with HLSL 2021

tint executable returned error: exit status 1
