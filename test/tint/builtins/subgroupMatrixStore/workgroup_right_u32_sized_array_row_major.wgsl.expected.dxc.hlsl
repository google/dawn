...
hlsl.hlsl:51:5: error: no matching member function for call to 'Store'
  m.Store(out5, ((0u / 2u) + ((asuint(int(0)) * 4u) / 2u)), ((16u * 4u) / 2u), MatrixLayout::RowMajor);
  ~~^~~~~
...
hlsl.hlsl:52:5: error: no matching member function for call to 'Store'
  m.Store(out6, ((0u / 2u) + ((asuint(int(0)) * 8u) / 2u)), ((16u * 8u) / 2u), MatrixLayout::RowMajor);
  ~~^~~~~
...
<built-in:hlsl>/dx/linalg.h:341:52: error: cannot initialize a parameter of type 'unsigned int const __attribute__((address_space(3))) (&)[1024]' with an lvalue of type 'vector<unsigned int, 2> __attribute__((address_space(3)))[1024]'
    __builtin_LinAlg_MatrixStoreToMemory(__handle, Arr, StartIdx, Stride,
                                                   ^~~
hlsl.hlsl:48:5: note: in instantiation of function template specialization 'dx::linalg::Matrix<dx::linalg::ComponentType::ComponentEnum::U32, 8, 8, dx::linalg::MatrixUse::MatrixUseEnum::B, dx::linalg::MatrixScope::MatrixScopeEnum::Wave>::Store<vector<unsigned int, 2>, 1024>' requested here
  m.Store(out1, asuint(int(0)), 16u, MatrixLayout::RowMajor);
    ^
...
<built-in:hlsl>/dx/linalg.h:341:52: error: cannot initialize a parameter of type 'unsigned int const __attribute__((address_space(3))) (&)[1024]' with an lvalue of type 'vector<unsigned int, 4> __attribute__((address_space(3)))[1024]'
    __builtin_LinAlg_MatrixStoreToMemory(__handle, Arr, StartIdx, Stride,
                                                   ^~~
hlsl.hlsl:50:5: note: in instantiation of function template specialization 'dx::linalg::Matrix<dx::linalg::ComponentType::ComponentEnum::U32, 8, 8, dx::linalg::MatrixUse::MatrixUseEnum::B, dx::linalg::MatrixScope::MatrixScopeEnum::Wave>::Store<vector<unsigned int, 4>, 1024>' requested here
  m.Store(out3, asuint(int(0)), 16u, MatrixLayout::RowMajor);
    ^
...
void main_inner(uint tint_local_index) {
  ...
  Matrix_right_u32_8x8 m = Matrix_right_u32_8x8::Splat(0u);
  m.Store(out0, asuint(int(0)), 16u, MatrixLayout::RowMajor);
  m.Store(out1, asuint(int(0)), 16u, MatrixLayout::RowMajor);
  m.Store(out2, ((0u / 4u) + ((asuint(int(0)) * 16u) / 4u)), ((16u * 16u) / 4u), MatrixLayout::RowMajor);
  m.Store(out3, asuint(int(0)), 16u, MatrixLayout::RowMajor);
  m.Store(out5, ((0u / 2u) + ((asuint(int(0)) * 4u) / 2u)), ((16u * 4u) / 2u), MatrixLayout::RowMajor);
  m.Store(out6, ((0u / 2u) + ((asuint(int(0)) * 8u) / 2u)), ((16u * 8u) / 2u), MatrixLayout::RowMajor);
}
