...
void main() {
  ...
  m.Store(out1, (0u + (select(v_3, 0u, 0u) * 8u)), (select(v_3, 16u, 4u) * 8u), MatrixLayout::ColMajor);
  uint v_4 = 0u;
  out2.GetDimensions(v_4);
  bool v_5 = (114u <= (v_4 / 16u));
  m.Store(out2, (0u + (select(v_5, 0u, 0u) * 16u)), (select(v_5, 16u, 2u) * 16u), MatrixLayout::ColMajor);
  uint v_6 = 0u;
  out3.GetDimensions(v_6);
  bool v_7 = (114u <= (v_6 / 16u));
  m.Store(out3, (0u + (select(v_7, 0u, 0u) * 16u)), (select(v_7, 16u, 2u) * 16u), MatrixLayout::ColMajor);
  uint v_8 = 0u;
  ...
  m.Store(out6, (0u + (select(v_11, 0u, 0u) * 8u)), (select(v_11, 16u, 4u) * 8u), MatrixLayout::ColMajor);
}
